...
export default function TechnicianDashboard() {
  ...
  useEffect(() => {
    if (technicianId) getCurrentLocation()
  }, [technicianId])

  useEffect(() => {
    if (technicianId) fetchAndSortIssues()
      ...
      if (data) {
        const withDistance: IssueWithDistance[] = data.map(issue => ({
          ...
          distance: (issue.latitude && issue.longitude && myLocation)
            ? calculateDistance(myLocation.lat, myLocation.lng, issue.latitude, issue.longitude)
            : 999999
        }))
        withDistance.sort((a, b) => (a.distance ?? 999999) - (b.distance ?? 999999))
  ...
  const checkAttendance = async (id: string) => {
    ...
    const { data } = await supabase.from('attendance').select('*')
      .eq('technician_id', id).eq('date', today).is('check_out', null).maybeSingle()
    setIsCheckedIn(!!data)
  }

  const handleCheckIn = async () => {
    if (!myLocation) { alert('Please enable location first'); getCurrentLocation(); return }
    const today = new Date().toISOString().split('T')[0]

    // Check if already checked in
    const { data: existing } = await supabase.from('attendance').select('id')
      .eq('technician_id', technicianId).eq('date', today).maybeSingle()
    if (existing) { alert('Already checked in today!'); setIsCheckedIn(true); return }

    const { error } = await supabase.from('attendance').insert({
      technician_id: technicianId,
      check_in: new Date().toISOString(),
      latitude: myLocation.lat, longitude: myLocation.lng, date: today
    })
    if (!error) {
      setIsCheckedIn(true)
  ...
  const handleCheckOut = async () => {
    const today = new Date().toISOString().split('T')[0]
    const { data } = await supabase.from('attendance').select('*')
      .eq('technician_id', technicianId).eq('date', today).is('check_out', null).maybeSingle()
    if (data) {
      const hours = (Date.now() - new Date(data.check_in).getTime()) / 3600000
      const { error } = await supabase.from('attendance').update({
        check_out: new Date().toISOString(), total_hours: hours
      }).eq('id', data.id)
      if (!error) {
  ...
  const handleStartIssue = async (e: React.MouseEvent, issueId: string) => {
    ...
    const { error } = await supabase.from('issues')
      .update({ status: 'in-progress', started_at: new Date().toISOString() }).eq('id', issueId)
    if (!error) fetchAndSortIssues()
  }
  ...
  const handleNavigateAll = () => {
    if (!myLocation) { alert('Please enable location first'); return }
    const issuesWithCoords = issues.filter(i => i.latitude && i.longitude)
    if (issuesWithCoords.length === 0) { alert('No issues have GPS coordinates'); return }
    const origin = `${myLocation.lat},${myLocation.lng}`
    if (issuesWithCoords.length === 1) {
      window.open(`https://www.google.com/maps/dir/?api=1&origin=${origin}&destination=${issuesWithCoords[0].latitude},${issuesWithCoords[0].longitude}&travelmode=driving`, '_blank')
    } else {
      const waypoints = issuesWithCoords.slice(0, -1).map(i => `${i.latitude},${i.longitude}`).join('|')
      const dest = `${issuesWithCoords[issuesWithCoords.length - 1].latitude},${issuesWithCoords[issuesWithCoords.length - 1].longitude}`
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-20">
      <header className="bg-white dark:bg-gray-800 border-b sticky top-0 z-10">
        <div style={{ padding: '10px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px', minWidth: 0 }}>
          {/* Left: logo + name */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', minWidth: 0, flex: 1 }}>
            <img src="/cautio_shield.webp" alt="Cautio" style={{ width: '28px', height: '28px', objectFit: 'contain', flexShrink: 0 }} />
            <div style={{ minWidth: 0 }}>
              <p style={{ fontWeight: 700, fontSize: '15px', lineHeight: 1.2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>My Tasks</p>
              {technicianName && <p style={{ fontSize: '11px', color: '#9ca3af', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{technicianName}</p>}
            </div>
          </div>
          {/* Right: icon buttons — compact */}
          <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexShrink: 0 }}>
            {technicianId && <NotificationBell userId={technicianId} />}
            <ThemeToggle />
            <button onClick={() => router.push('/technician/map')} style={{ width: '32px', height: '32px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'inherit' }}>
              <Map className="h-4 w-4" />
            </button>
            <button onClick={getCurrentLocation} disabled={fetchingLocation} style={{ width: '32px', height: '32px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'inherit', opacity: fetchingLocation ? 0.5 : 1 }}>
              <Locate className={`h-4 w-4 ${fetchingLocation ? 'animate-spin' : ''}`} />
            </button>
            <button onClick={() => fetchAndSortIssues()} style={{ width: '32px', height: '32px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'inherit' }}>
              <RefreshCw className="h-4 w-4" />
            </button>
            <button onClick={handleLogout} style={{ width: '32px', height: '32px', borderRadius: '8px', border: '1px solid rgba(239,68,68,0.2)', background: 'rgba(239,68,68,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#ef4444' }}>
      ...
      <main className="px-4 py-5 space-y-5 max-w-2xl mx-auto">
          ...
          <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-3 text-sm">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-green-700 dark:text-green-300">
                ...
                <span className="font-medium">{myLocation.lat.toFixed(4)}, {myLocation.lng.toFixed(4)}</span>
              </div>
              {issuesWithGPS > 0 && (
        ...
        {issuesWithoutGPS > 0 && (
          <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3">
            <div className="flex items-center gap-2 text-yellow-700 dark:text-yellow-300 text-sm">
              ...
              <span>{issuesWithoutGPS} issue(s) missing GPS - distance unknown</span>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-3">
          <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => router.push('/technician/attendance')}>
            <CardContent className="pt-4 pb-4 text-center">
              <Calendar className="h-6 w-6 text-purple-600 mx-auto mb-1" />
              <p className="text-xs font-medium">Attendance</p>
            </CardContent>
          </Card>
          <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => router.push('/technician/inventory')}>
            <CardContent className="pt-4 pb-4 text-center">
              <Package className="h-6 w-6 text-teal-600 mx-auto mb-1" />
              <p className="text-xs font-medium">Inventory</p>
            </CardContent>
          </Card>
          <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => router.push('/technician/profile')}>
            <CardContent className="pt-4 pb-4 text-center">
                  ...
                  <CardContent className="p-4">
                    ...
                    {issue.distance !== undefined && issue.distance < 999999 ? (
                      ...
                      <div className="bg-gray-100 border border-gray-300 rounded-md px-3 py-2 mb-3">
                        <p className="text-xs text-gray-600 flex items-center gap-2">
                          <AlertCircle className="h-3 w-3" />No GPS data
                        </p>
                      </div>
                    )}
                    ...
                    <div className="space-y-1.5 mb-4">
                      {(issue.city || issue.location) && (
                        <p className="text-sm text-muted-foreground flex items-center gap-1.5">
                          <MapPin className="h-3.5 w-3.5" />{issue.city}{issue.location && ` - ${issue.location}`}
                        </p>
                      )}
                      {issue.poc_name && (
                        <p className="text-sm text-muted-foreground flex items-center gap-1.5">
                          <Phone className="h-3.5 w-3.5" />{issue.poc_name}{issue.poc_number && ` • ${issue.poc_number}`}
                        </p>
                      )}
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <Button size="sm" variant="outline" onClick={(e) => { e.stopPropagation(); router.push(`/technician/issues/${issue.id}`) }}>
                        <Camera className="h-4 w-4 mr-2" />Details
                      </Button>
